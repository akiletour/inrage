...
<Callout>Comment organiser son thème WordPress Sage 9 pour créer et gérer des blocs Gutenberg avec Advanced Custom Fields (ACF).</Callout>
...
<strong class="text-white">$name</strong> = correspondra au slug de notre bloc<br />
<strong class="text-white">$icon</strong> = l’îcone qui sera utilisé dans notre éditeur Gutenberg, vous pouvez choisir une icone grace aux [Dashicons de WordPress](https://developer.wordpress.org/resource/dashicons/).
...
<strong>$icon</strong> : et on va choisir une présentation de témoignage avec nos Dashicons
