...
function LinkItem({
  title, href, isActive = false, children, closePrimaryNav,
}: LinkProps) {
  if (children) {
    return (
      <div className="group">
        <Link href={href}>
          <a onClick={() => closePrimaryNav()} className={`uppercase block text-lg pt-2 lg:py-2 group-hover:text-orange ${isActive ? 'text-orange' : 'text-white'}`}>{title}</a>
        </Link>
  ...
  return (
    <Link href={href}>
      <a onClick={() => closePrimaryNav()} className={`block pt-2 lg:pt-0 uppercase text-lg ${isActive ? 'text-orange' : 'text-white hover:text-orange'}`}>{title}</a>
    </Link>
  );
}
...
export default function NavPrimary({ isSticky = false }: Props) {
  ...
  const n1TitleClassnames = 'uppercase text-white text-sm lg:text-base font-bold block hover:text-orange my-1';
        ...
        <div className="container flex items-center justify-between py-2">
          ...
          >
            <svg className="w-4 h-4" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
              ...
              <path fill="currentColor" d="M2 13.5h14V12H2v1.5zm0-4h14V8H2v1.5zM2 4v1.5h14V4H2z" />
            </svg>
          </button>

          <div
            className={`${width < 1025 && isOpen
              ? 'block lg:flex bg-gray-darker divide-y divide-gray-dark space-y-2 text-center flex-col absolute top-full left-0 w-full'
              : 'hidden relative space-x-3'} lg:flex items-center`}
          >
            <LinkItem closePrimaryNav={() => setOpen(false)} isActive={pathname === RouteLink.aboutMe} href={RouteLink.aboutMe} title="À propos de moi" />
            <LinkItem closePrimaryNav={() => setOpen(false)} isActive={pathname === RouteLink.prestations} href={RouteLink.prestations} title="Prestations">
              <div className="flex justify-evenly lg:justify-start">
                <div>
                  <Link href={RouteLink.prestationWeb}>
                    <a onClick={() => setOpen(false)} className={n1TitleClassnames}>Création de site</a>
                  </Link>
                  <ul className="lg:list-disc lg:pl-2 marker:text-orange-dark">
                    <li>
                      <Link href={RouteLink.prestationWordPress}>
                        <a onClick={() => setOpen(false)} className={n2TitleClassnames}>WordPress</a>
                      </Link>
                    </li>
                    <li>
                      <Link href={RouteLink.prestationPrestashop}>
                        <a onClick={() => setOpen(false)} className={n2TitleClassnames}>Prestashop</a>
                      </Link>
                    </li>
                    <li>
                      <Link href="#">
                        <a onClick={() => setOpen(false)} className={n2TitleClassnames}>Symfony</a>
                      </Link>
                    </li>
                  </ul>
                </div>
            ...
            <LinkItem closePrimaryNav={() => setOpen(false)} isActive={pathname === RouteLink.portfolio} href={RouteLink.portfolio} title="Portfolio">
            ...
            <Link href={RouteLink.contact}>
              <a className="mt-2 lg:mt-0 inline-flex py-1 px-2 bg-orange uppercase text-gray-darker font-medium">Contactez-moi</a>
            </Link>

            <div className="block lg:hidden text-3xl text-orange font-bold pt-2">06 51 89 89 17</div>
            <button type="button" className="block lg:hidden w-full" onClick={toggleNavPrimary}>
              <div className="bg-gray-darker -mt-1">
                <svg className="translate translate-y-[30px] block w-full h-auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 415 76">
                  <defs><path id="a" d="M291.516 48.222L414.422.36v75h-414z" /></defs>
                  <g fill="none" fillRule="evenodd">
                    <use fill="none" xlinkHref="#a" />
                    <path fill="#E57E21" d="M414.422 1v51.282L312 40.886z" />
                    <path fill="#FFF" fillRule="nonzero" d="M402.422 23.273l-1.914-1.914-7.586 7.586-7.587-7.586-1.913 1.914 7.586 7.586-7.586 7.586 1.913 1.914 7.587-7.586 7.586 7.586 1.914-1.914-7.587-7.586z" />
                  </g>
                </svg>
              </div>
            </button>
