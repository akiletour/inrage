...
export default function TableLine({ title, content, values }: Props) {
  return (
    <div className="flex items-center flex-wrap">
      <div className="w-full mb-2 lg:mb-0 lg:w-auto lg:flex-1 pl-3 pr-3">
        <h3 className="text-2xl text-orange font-medium">{title}</h3>
        <p className="text-white">{content}</p>
      </div>

      {values.map((value) => (
        <div className="lg:w-25 flex-1 md:flex-none flex-col text-center text-white text-lg leading-6 flex-none flex items-center justify-center" key={`title-${value}`}>
          {typeof value === 'string'
