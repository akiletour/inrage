...
Connectez-vous en SSH à votre serveur. Les fichiers corrompus se trouvent dans `config/xml/`. Il faut remplacer leur contenu par du XML valide mais vide, en attendant que les serveurs Addons reviennent en ligne :

```bash
cd /chemin/vers/votre/prestashop
for f in config/xml/*.xml; do
  echo '<?xml version="1.0" encoding="UTF-8"?><modules></modules>' > "$f"
done
```

Si vous n'avez que du FTP, ouvrez chaque fichier `.xml` du dossier `config/xml/` et remplacez son contenu par :
